      ...
      {items.map((item) => (
        <div key={item.label} className="px-2 py-5 text-center">
          <dd className="overflow-hidden">
            <span
              key={item.value}
              className="digit-roll block font-display text-2xl font-semibold tabular-nums text-navy sm:text-3xl"
            >
              {now === null ? "—" : String(item.value).padStart(2, "0")}
            </span>
          </dd>
          <dt className="mt-1 text-[10px] uppercase tracking-[0.24em] text-muted-foreground">
            {item.label}
          </dt>
        </div>
      ))}
